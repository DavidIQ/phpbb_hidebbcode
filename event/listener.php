...
class listener implements EventSubscriberInterface
{
    ...
    static public function getSubscribedEvents()
    {
        return array(
            ...
            'core.display_custom_bbcodes_modify_row' => 'set_helpline'
        );
    }
    ...
    {
        $this->lang->add_lang('hide_bbcode', 'marcovo/hideBBcode');

        $s_hidden_desc = '';
        $s_hidden_attach = $this->lang->lang('HIDEBB_MESSAGE_HIDDEN_ATTACH');

        if ($this->config['hidebbcode_unhide_reply'] && $this->config['hidebbcode_unhide_tfp'])
        {
            $this->s_helpline = $this->lang->lang('HIDEBB_HIDE_HELPLINE_REPLY_THANK');
            $s_hidden_desc = $this->lang->lang('HIDEBB_MESSAGE_HIDDEN_DESCRIPTION_REPLY_THANK');
            $s_hidden_attach .= ' ' . $this->lang->lang('HIDEBB_MESSAGE_HIDDEN_ATTACH_REPLY_THANK');
        } else if ($this->config['hidebbcode_unhide_reply'])
        {
            $this->s_helpline = $this->lang->lang('HIDEBB_HIDE_HELPLINE_REPLY');
            $s_hidden_desc = $this->lang->lang('HIDEBB_MESSAGE_HIDDEN_DESCRIPTION_REPLY');
            $s_hidden_attach .= ' ' . $this->lang->lang('HIDEBB_MESSAGE_HIDDEN_ATTACH_REPLY');
        } else if ($this->config['hidebbcode_unhide_tfp'])
        {
            $this->s_helpline = $this->lang->lang('HIDEBB_HIDE_HELPLINE_THANK');
            $s_hidden_desc = $this->lang->lang('HIDEBB_MESSAGE_HIDDEN_DESCRIPTION_THANK');
            $s_hidden_attach .= ' ' . $this->lang->lang('HIDEBB_MESSAGE_HIDDEN_ATTACH_THANK');
        }

        $this->template->assign_vars([
            'HIDEBB_MESSAGE_HIDDEN_DESCRIPTION'     => $s_hidden_desc,
            'HIDEBB_MESSAGE_HIDDEN_ATTACH'          => $s_hidden_attach
        ]);
    }

    public function set_helpline($event)
    {
        if ($event['row']['bbcode_helpline'] === 'HIDEBB_HIDE_HELPLINE')
        {
            $row = $event['row'];
            $custom_tags = $event['custom_tags'];
            $row['bbcode_helpline'] = $this->s_helpline;
            $custom_tags['BBCODE_HELPLINE'] = $this->s_helpline;
            $event['row'] = $row;
     ...
     *
     * @param int $post_id the posting id
     */
    ...
    public function check_user_posted_pm($event)
    {
        ...
        {
            $post['message_text'] = preg_replace('#\[hide:' . $bbcode_uid . '\].*?\[/hide:' . $bbcode_uid . '\]#is', '{{' . $this->lang->lang('HIDEBB_HIDDEN_MESSAGE') . "}}\n", $post['message_text']);
        } else
        {
    ...
    public function topic_review_modify_row($event)
    {
        if (!$this->unhide_in_post($event['row']['post_id']))
        {
            ...
            $post_row['DECODED_MESSAGE'] = preg_replace("#\[hide\].*?\[/hide\]#is", '{{' . $this->lang->lang('HIDEBB_HIDDEN_MESSAGE') . "}}\n", $post_row['DECODED_MESSAGE']);
    ...
    public function posting_modify_template_vars($event)
    {
        ...
        if (!$this->unhide_in_post($event['post_id']) && in_array($event['mode'], array('reply', 'quote')) && !$event['preview'] && $draft_id == 0)
        {
            ...
            $page_data['MESSAGE'] = preg_replace("#\[hide\].*?\[/hide\]#is", '{{' . $this->lang->lang('HIDEBB_HIDDEN_MESSAGE') . "}}\n", $page_data['MESSAGE']);
    ...
    public function TFP_delete_thanks_before($event)
    {
        ...
        if (strpos($post_text, '[hide:') !== false)
        {
            ...
            trigger_error($this->lang->lang('HIDEBB_TFP_NO_DELETE') . '<br /><br />' . $this->lang->lang('RETURN_POST', '<a href="' . append_sid($phpbb_root_path . "viewtopic." . $phpEx, "f=$forum_id&amp;p=$post_id#p$post_id") . '">', '</a>'));
        }
